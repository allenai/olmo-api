...
class TestLabelEndpoints(base.IntegrationTest):
    ...
    def runTest(self):
        ...
        for b in bad:
            r = requests.post(f"{self.origin}/v2/label", headers=self.auth(u1), json=b)
            assert r.status_code == 400

        # Create a label
        r = requests.post(f"{self.origin}/v2/label", headers=self.auth(u1), json={
            "rating": 1,
            "message": m1["id"]
        })
        r.raise_for_status()
        ...
        # Make sure /v2/message/:id returns the label
        r = requests.get(f"{self.origin}/v2/message/{m1['id']}", headers=self.auth(u1))
        r.raise_for_status()
        assert len(r.json()["labels"]) == 1
        assert r.json()["labels"][0] == l1

        # Make sure /v2/message/:id doesn't return the label for u2
        r = requests.get(f"{self.origin}/v2/message/{m1['id']}", headers=self.auth(u2))
        r.raise_for_status()
        ...
        r = requests.get(f"{self.origin}/v2/messages", headers=self.auth(u2))
        r.raise_for_status()
        for m in r.json():
            if m["id"] != m1["id"]:
                continue
        ...
        # Verify /v2/label/:id
        r = requests.get(f"{self.origin}/v2/label/{l1['id']}", headers=self.auth(u1))
        r.raise_for_status()
        assert l1 == r.json()
        ...
        r = requests.post(f"{self.origin}/v2/label", headers=self.auth(u2), json={
            "rating": -1,
            "message": m2["id"],
        ...
        r = requests.get(f"{self.origin}/v2/labels", headers=self.auth(u1))
        r.raise_for_status()
        ...
        r = requests.get(f"{self.origin}/v2/labels?message={m1['id']}", headers=self.auth(u1))
        r.raise_for_status()
        assert r.json() == [l1]

        # Verify filtering by creator
        r = requests.get(f"{self.origin}/v2/labels?creator={u2['client']}", headers=self.auth(u1))
        r.raise_for_status()
        ids = [l["id"] for l in r.json()]
        assert l2["id"] in ids
        ...
        r = requests.delete(f"{self.origin}/v2/label/{l1['id']}", headers=self.auth(u2))
        assert r.status_code == 403

        # Delete a label
        r = requests.delete(f"{self.origin}/v2/label/{l1['id']}", headers=self.auth(u1))
        r.raise_for_status()
        assert r.json()["deleted"] is not None

        # Make sure /v2/message/:id no longer returns it.
        r = requests.get(f"{self.origin}/v2/message/{m1['id']}", headers=self.auth(u1))
        r.raise_for_status()
        assert len(r.json()["labels"]) == 0

        # ...and that /v2/messages does the same
        r = requests.get(f"{self.origin}/v2/messages", headers=self.auth(u1))
        r.raise_for_status()
        for m in r.json():
            if m["id"] != m1["id"]:
        ...
        r = requests.get(f"{self.origin}/v2/labels?deleted", headers=self.auth(u1))
        r.raise_for_status()
        ids = [l["id"] for l in r.json()]
        assert l1["id"] in ids
        assert l2["id"] in ids
    ...
    def tearDown(self):
        ...
        for (id, user) in self.labels:
            r = requests.delete(f"{self.origin}/v2/label/{id}", headers=self.auth(user))
            r.raise_for_status()
