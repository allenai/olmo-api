...
function(flaskApiImage, cause, sha, env='prod', branch='', repo='', buildId='', safetyWorkerImage, fastApiImage)
    ...
    local numApiSafetyWorkerReplicas = if env == 'prod' then config.workerReplicas.prod else 1;

    local apiSafetyWorkerDeployment = {
        apiVersion: 'apps/v1',
        kind: 'Deployment',
        metadata: {
            labels: labels,
            name: apiSafetyWorkerFQN,
            namespace: namespaceName,
            annotations: annotations + {
                'kubernetes.io/change-cause': cause
            }
        ...
        spec: {
            ...
            strategy: {
                type: 'RollingUpdate',
                rollingUpdate: {
                    maxSurge: numApiSafetyWorkerReplicas // This makes deployments faster.
                }
            },
            revisionHistoryLimit: 3,
            replicas: numApiSafetyWorkerReplicas,
            selector: {
                matchLabels: apiSafetyWorkerSelectorLabels
            },
            template: {
                metadata: {
                    name: apiSafetyWorkerFQN,
                    namespace: namespaceName,
                    // It's OK if multiple workers get scheduled onto one node so we don't include anti-affinity labels here
                    labels: apiSafetyWorkerPodLabels,
                    annotations: annotations
                },
                spec: {
                    nodeSelector: nodeSelector,
                    ...
                    containers: [
                        {
                            ...
                            # with you to craft the right livenessProbe.
                            readinessProbe: {
                                httpGet: apiSafetyWorkerHealthCheck,
                                periodSeconds: 10,
                                failureThreshold: 3
                            },
                            startupProbe: {
