...
class TestPromptTemplateEndpoints(base.IntegrationTest):
    ...
    def runTest(self):
        ...
        r = requests.post(
            f"{self.origin}/v3/templates/prompt",
            headers=self.auth(u1),
            json={"name": "Test Prompt #1", "content": "Murphy is a good dog who likes to "},
        )
        r.raise_for_status()
        p1 = r.json()
        ...
        r = requests.post(
            ...
            json={"name": "Test Prompt #2", "content": "Grasshopper is a silly horse that likes to "},
        )
        r.raise_for_status()
        p2 = r.json()
        ...
        # Update p1.
        r = requests.patch(
            f"{self.origin}/v3/templates/prompt/{p1['id']}",
            headers=self.auth(u1),
            json={
