...
async def get_general_mcp_tools() -> list[Ai2ToolDefinition]:
    ...
    general_mcp_servers = [server for server in MCP_SERVERS if _is_mcp_server_for_general_use(server)]

    if not general_mcp_servers:
        return []

    server_tool_lists = await asyncio.gather(
        *[list_mcp_server_tools(server) for server in general_mcp_servers], return_exceptions=True
    )
...
async def get_tools_from_mcp_servers(mcp_server_ids: set[str]) -> list[Ai2ToolDefinition]:
    ...
    server_tool_lists = await asyncio.gather(
        *[list_mcp_server_tools(server) for server in matching_servers], return_exceptions=True
    )
