...
class UploadedFile(FileStorage):
    ...
    def __get_pydantic_core_schema__(cls, source_type: Any, handler: GetCoreSchemaHandler) -> CoreSchema:
        return core_schema.no_info_before_validator_function(cls.validate, core_schema.AnySchema(type="any"))
...
def get_annotated_models(
    func: Callable,
) -> tuple[Optional[str], Optional[list[type[BaseModel]]], Optional[list[type[BaseModel]]]]:
    request_models = None
    ...
    view_model_args = [
        ...
        if v and k != "return" and ((isclass(v) and issubclass(v, BaseModel)) or is_union_of_model_sublclasses(v))
    ]

    if len(view_model_args) > 1:
        raise Exception(
            f"Too many model arguments specified for {func.__name__}. Could not determine which to map to request body"
        )
    if len(view_model_args) == 1 and (request_annotation := func.__annotations__.get(view_model_args[0])):
