...
class BaseModelResponse(APIInterface):
...
class TextOnlyModelResponse(BaseModelResponse):
...
def none_to_no_requirements_validator(
    value: FileRequiredToPromptOption | None,
) -> FileRequiredToPromptOption:
    if value is None:
...
class MultiModalModelResponse(BaseModelResponse):
...
class ModelResponse(RootModel):
    root: Annotated[
        TextOnlyModelResponse | MultiModalModelResponse,
        Field(discriminator="prompt_type"),
    ]


class ModelListResponse(RootModel):
    root: list[ModelResponse]
