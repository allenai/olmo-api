...
UpdateModelConfigRequest = UpdateTextOnlyModelConfigRequest | UpdateMultiModalModelConfigRequest
...
def update_model_config(
    ...
    with session_maker.begin() as session:
        updated_model = session.scalar(
            update(ModelConfig).where(ModelConfig.id == model_id).values(request.model_dump()).returning(ModelConfig),
        )

        return ResponseModel.model_validate(updated_model) if updated_model is not None else None
