...
def get_model_configs(
    ...
    with session_maker.begin() as session:
        ...
        model_validation_context = ModelValidationContext(
            should_show_internal_models=include_internal_models
        )

        mapped_models = ModelResponse.model_validate(
            results, from_attributes=True, context=model_validation_context
        )
        ...
        for mapped_model in mapped_models.root:
            mapped_model.available_tools = (
                available_tool_list_type_adapter.validate_python(
