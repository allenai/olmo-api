...
class DateTimeUTC(TypeDecorator[datetime.datetime]):
    ...
    def process_bind_param(self, value: Optional[datetime.datetime], dialect: Dialect) -> Optional[datetime.datetime]:  # noqa: ARG002, PLR6301
        if value is None:
            return value
        if not value.tzinfo:
    ...
    def process_result_value(self, value: Optional[datetime.datetime], dialect: Dialect) -> Optional[datetime.datetime]:  # noqa: ARG002, PLR6301
        if value is None:
            return value
