# HACK: This is a fix I made on top of flask_pydantic_api. Remove this when form data list handling is merged and released
...
try:  # noqa: SIM105
    from pydantic_enhanced_serializer import render_fieldset_model  # type: ignore
except ImportError:
    pass
...
def get_request_args(
    ...
    request_is_multipart = (request.mimetype or "").lower().startswith("multipart/form-data")

    if for_models:
        if len(for_models) == 1 and model_has_uploaded_file_type(for_models[0]) and not request_is_multipart:
    ...
    if merge_path_parameters and view_kwargs and request.url_rule and request.url_rule.arguments:
        args.update({k: v for k, v in view_kwargs.items() if k in request.url_rule.arguments})
    ...
    def wrap(
        ...
        def wrapped_endpoint(*args: P.args, **kwargs: P.kwargs) -> Response:
            ...
            try:
                ...
                if body and render_fieldset_model and request_fields_name and request_fields_name in body:
                    fieldsets_input = body.pop(request_fields_name, [])
                    fieldsets = fieldsets_input.split(",") if isinstance(fieldsets_input, str) else fieldsets_input
                ...
                if current_app.config.get("FLASK_PYDANTIC_API_RENDER_ERRORS", False):
                    ...
                    response.status_code = current_app.config.get("FLASK_PYDANTIC_API_ERROR_STATUS_CODE", 400)
                    return response
            ...
            try:
                ...
                if isinstance(result, BaseModel):
                    ...
                    if success_status_code_by_response_model:
                        status_code = success_status_code_by_response_model.get(result.__class__, success_status_code)
                ...
                raise Exception(
                    f"pydantic model error on api response serialization; endpoint: {request.endpoint}; error: {e!s}"
                )
