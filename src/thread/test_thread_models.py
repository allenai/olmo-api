...
from src.thread.thread_models import BAD_TOOL_NAME, FlatMessage, InferenceOptionsResponse, ToolCall
...
def test_truncates_content_if_required():
    message = FlatMessage(
        ...
        creator="test-user",
        tool_calls=[
            ToolCall(tool_name=BAD_TOOL_NAME, args={}, tool_call_id="bad_tool_call", tool_source=ToolSource.MCP)
        ],
    )
...
def test_keeps_content_if_tool_call_name_does_not_match():
    message = FlatMessage(
        ...
        creator="test-user",
        tool_calls=[
            ToolCall(tool_name="good_tool_call", args={}, tool_call_id="good_tool_call", tool_source=ToolSource.MCP)
        ],
    )
